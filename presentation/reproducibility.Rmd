...
- all files should be humanly readable
...
1. For medium-sized projects  separate code in multiples by functionality
...
Write always a README file with information on: 
	\begin{itemize}
	    \item what/what order to run the code files
	    \item what tools you need (e.g., \LaTeX, \texttt{Make}, etc) 
	    \item README files are text files---often with some structure (markup language). Example are markdown and R markdown. 
	\end{itemize}
\end{column}
\begin{column}{.5\textwidth}
    \dirtree{%
...
- For functions you might want to specify the \alert{type} of input and the type of output
...
Only when for packages (short-cut in R-studio): creates a skeleton
...
- use of functions (e.g., to loop over histograms as descriptives)
...
- automatically save (regression) output with `stargazer` or `texreg`
...
\begin{column}{.5\textwidth}
	\begin{center}
			\includegraphics[width = 0.8\textwidth]{../fig/version}
	\end{center}
\end{column}
\begin{column}{.5\textwidth}
    \begin{itemize}
    ...
    \end{itemize}
    [Note 1: RStudio has Git functionality]\newline
    [Note 2: a bit like Dropbox]
  \end{column}
\end{columns}
